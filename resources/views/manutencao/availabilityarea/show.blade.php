                ...
                <tbody>
                    @foreach (\App\Models\TypeEquipment::all() as $item)
                        <tr>
                            ...
                            <td>{{count($item->equipment->where('area_id',$area->id))}}</td>
                            @if (count($item->equipment->where('area_id',$area->id)) == 0)
                                <td>0%</td>
                            @else
                            <td>{{round(count($item->equipment->where('area_id',$area->id)->where('status',1))*100/count($item->equipment->where('area_id',$area->id)),2)}} %</td>
                            @endif
                            
                            <td style="color: green">{{count($item->equipment->where('area_id',$area->id)->where('status',1))}}</td>
                            <td style="color: red">{{count($item->equipment->where('area_id',$area->id)->where('status',0))}}</td>
